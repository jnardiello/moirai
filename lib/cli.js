...
const { checkForUpdateAndMaybeInstall } = require('./update-check');

const USAGE = `Moirai

Usage:
  moirai [--root <path>] [--port <port>] [--host <host>] [--no-open] [--no-update-check]
  moirai init [--root <path>] [--port <port>] [--host <host>] [--no-open] [--no-update-check]
  moirai doctor [--root <path>] [--no-update-check]
  moirai --help
  moirai --version
`;
...
async function runCli(argv = process.argv.slice(2), io = {}) {
  ...
  if (options.updateCheck) {
    const updateResult = await checkForUpdateAndMaybeInstall({
      env,
      stderr,
      stdin: io.stdin || process.stdin,
      currentVersion: packageVersion(),
      packageRoot: path.resolve(__dirname, '..'),
      rerunArgs: [process.argv[1], ...argv],
      ...(io.updateCheckOptions || {}),
    });
    if (updateResult.abort || updateResult.rerun) {
